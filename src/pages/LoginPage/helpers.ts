...
const INVALID_NICKNAME = 'Nickname must have 5 more symbols';

export const checkPhone = (phone: string, mask: string) => {
  return phone.length === mask.length ? '' : INVALID_PHONE_NUMBER_ERROR;
};

export const checkDualCode = (countryName: string) => {
  return countryName === INVALID_COUNTRY_CODE || countryName === NO_COUNTRY ? INVALID_DUAL_CODE_ERROR : '';
};
